...
const UpdateJobPostForm = (job) => {
    ...
    const [formData, setFormData] = useState({
        ...
        recruitmentQuota: job.job.recruitmentQuota,
        salary: job.job.salary,
        location: job.job.location,
        informationToInterviewer: job.job.informationToInterviewer
    });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setLoading(true)
            const { data } = await axios.post(`http://localhost:5000/api/jobs/update/${id}` , formData , config)
            console.log(data);
            setLoading(false)
            setShowAlert(true)
            job.setModalIsOpen(false)
        }
        catch (err) {
